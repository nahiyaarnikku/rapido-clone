...
const CaptainSearch = ({ rideType, pickup, dropoff }) => {
  ...
  useEffect(() => {
    const fetchCaptain = async () => {
      ...
      try {
        const response = await fetch(`/api/captains/nearby?rideType=${rideType}&pickup=${pickup}&dropoff=${dropoff}`);
        if (!response.ok) {
          throw new Error('Failed to fetch captains');
        }
        const data = await response.json();
        
        // Assuming data.captains is an array and we take the first one for simplicity
        if (data.captains.length > 0) {
          ...
          setCaptainLocation(foundCaptain.location); // Assuming location is part of the captain data
        } else {
          setCaptain(null);
        }
      } catch (err) {
